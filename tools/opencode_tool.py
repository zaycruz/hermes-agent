...
def _run_task(
    ...
    for line in result.stdout.strip().split("\n"):
        ...
        try:
            ...
            # Extract useful parts from events
                ...
                if ptype == "text":
                    ...
                    if state.get("status") == "completed":
                        tool_results.append({
                            ...
                            "output": state.get("output", "")[:2000],
                        })

            elif etype == "session.diff":
                file_diffs.extend(event.get("properties", {}).get("diffs", []))

            elif etype == "session.idle":
                session_info = event.get("properties", {})

            elif etype == "session.error":
                errors.append(event.get("properties", {}))

        except json.JSONDecodeError:
            # Non-JSON output (status messages, etc.)
    ...
    if tool_results:
        # Include last 10 tool results to keep context manageable
...
registry.register(
    ...
    requires_env=[],
    is_async=False,
    description="Dispatch coding tasks to OpenCode's multi-agent harness (Sisyphus, Hephaestus, Oracle, etc.)",
    emoji="\U0001f5a5\ufe0f",  # desktop computer emoji
)
